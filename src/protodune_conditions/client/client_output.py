...
def verbose(ctx: typer.Context, result: CompletedProcess) -> None:
    details_panel_title_text = Text(
        text=f"Command Details: <{ctx.command_path}>", style="bright_magenta"
    )
    results_panel_title_text = Text(
        text=f"Command Results: <{ctx.command_path}>", style="bright_magenta"
    )
    result_text = Padding(
    ...
    options_table_title = Text(
        text="CLI Options", style=Style(color="bright_yellow", italic=True)
    )
    ...
    panel_group = Group(
        Panel(renderable=padded_table, title=details_panel_title_text),
        Panel(
            renderable=result_text,
