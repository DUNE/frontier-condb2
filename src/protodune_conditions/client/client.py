...
class Client:
    ...
    def get_data(self) -> subprocess.CompletedProcess[str]:
        ...
        subprocess.run(
            args=f"{self.state.frontier_client_path} {connect_string}",
            env=env,
            capture_output=True,
            check=True,
            shell=True,
        ...
        result: subprocess.CompletedProcess[str] = subprocess.run(
            args="mv 'get?folder=pdunesp.run_conditionstest&t0=28650&t1=28655' pdunesp.run_conditionstest.csv \
                && echo 'Wrote fetched data to: pdunesp.run_conditionstest.csv'",
            env=env,
            capture_output=True,
            check=True,
            shell=True,
